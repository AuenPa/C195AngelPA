...
public class AddAppointmentController implements Initializable {
    ...
    public void saveAddAppointment(ActionEvent event) throws IOException {
        ...
        LocalDateTime localTStart = addStartCombine;

        ZonedDateTime currentLocalTime = localTStart.atZone(localTZone);
        ...
        LocalDateTime localTEnd = addEndCombine;
        ZonedDateTime currentLocalTimeE = localTEnd.atZone(localTZone);
        ...
        if( !(checkStart && checkEnd) ) {
            ...
            String SB = "Business hours between 8am - 10pm est\n" +
                    ...
                    "\nYour time rn is " + dtf.format(LocalTime.now()) +
...
//+ ZonedDateTime.ofInstant(Instant.now(), ZoneId.of("America/New_York")
    ...
    public static boolean businessHoursCompare(LocalDateTime timeToCheck) {
        ZoneId eastT = ZoneId.of("America/New_York");
        ZoneId localTZone = ZoneId.systemDefault();
        LocalDateTime localTStart = timeToCheck;
        ZonedDateTime currentLocalTime = localTStart.atZone(localTZone);
        ZonedDateTime currentEasternTime = currentLocalTime.withZoneSameInstant(eastT);
        ...
        return zonedEastToLDT.isAfter(ChronoLocalDateTime.from(eastStart)) && zonedEastToLDT.isBefore(ChronoLocalDateTime.from(eastEnd));
    }
